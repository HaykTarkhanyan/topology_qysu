...
% Նկատենք, որ ներքին կետի սահմանման մեջ կարելի էր պահանջել, որ $U_x$-ը լինի $x$ կետի բաց շրջակայք։ Այն, որ սահմանման այդ երկու տարբերակները համարժեք են միմյանց, հետևում է կետի շրջակայքի սահմանումից: (\textbf{հիմնավորե՛ք})։
% %\par \vspace{4.5pt} \textbf{Ներքնամասի հատկությունները․}
% \paragraph*{Ենթաբազմության ներքինի հատկությունները․}
% \begin{enumerate}
...
% Սահմանումներից հետևում է՝ \red{chem jokum int-y jnjel te che}
% \[
% \inter A \cap \exter A = \inter A \cap \partial A = \exter A \cap \partial A = \varnothing,
% \]
% \[
...
% % example 2
% \begin{example} \label{Օրինակ 1։}
% Դիցուք $A$-ն հետևյալ $(0,1),[0,1), (0,1], [0,1]$ ենթաբազմություններից որևէ մեկն է թվային ուղղի սովորական տոպոլոգիայում։ Ապա $ \inter A = (0, 1)$, $\exter A = (-\infty, 0) \cup (1, +\infty)$, $ \partial A = \{0, 1\}$ (\textbf{հիմնավորե՛ք})։
% \end{example}
...
% % example 4Անցումը $M$ ենթաբազմությունից $\widebar{M}$-ին, այսինքն $M\mapsto \widebar{M}$ համադրումը կոչվում է \textbf{փակման գոր\-ծո\-ղու\-թյուն տոպոլոգիական տարածությունում}։
% % \end{definition}
